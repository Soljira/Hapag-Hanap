    ...
    if (availableIngredients && selectedIngredientsContainer) {
      ...
      async function searchRecipes() {
          const selectedIngredients = Array.from(
              document.querySelectorAll('#selected-ingredients input[type="checkbox"]')
          ).map((checkbox) => checkbox.value);
      
          if (selectedIngredients.length === 0) {
              recipesContainer.innerHTML = '<div class="empty-state">Select ingredients to find recipes</div>';
              return;
          }
          ...
          try {
              const minMatch = Math.max(3, Math.floor(selectedIngredients.length * 0.5));
